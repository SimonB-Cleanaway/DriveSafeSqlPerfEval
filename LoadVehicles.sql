declare @bu table(Code nvarchar(20), Name nvarchar(20))
insert into @bu(Code, Name) values ('BU1', 'Here'), ('BU2', 'There')
insert into BusinessUnit(Code, Name) select Code, Name from @bu bu  where not exists (select 1 from BusinessUnit where Code = bu.Code)

declare @eq table(Code nvarchar(20), Name nvarchar(20))
insert into @eq(Code, Name) values ('EQ1', 'This'), ('EQ2', 'That')
insert into Equipment(Code, Name) select Code, Name from @eq eq  where not exists (select 1 from Equipment where Code = eq.Code)
...
insert into NotificationLevel(Code, Name) select Code, Name from @nl nl  where not exists (select 1 from NotificationLevel where Code = nl.Code)
...
insert into ValidationRule(Code, Name, DefaultNotificationLevelId) select nr.Code, nr.Name, nl.NotificationLevelId  from @nr nr inner join NotificationLevel nl on nr.DefLevel = nl.Code where not exists (select 1 from ValidationRule where Code = nr.Code)
...
insert into Vehicle(VehicleNo, BusinessUnitId) select v.VehicleNo, b.BusinessUnitId from @v v inner join BusinessUnit b on v.BusUnit = b.Code where not exists (select 1 from Vehicle where VehicleNo = v.VehicleNo)
